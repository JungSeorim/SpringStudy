...
//    화면에서 4개 정보 입력 후 form태그로 전송한다
    ...
    @GetMapping("input")
    public void input(ProductVO productVO){
        log.info("input..............");
    }

    @GetMapping("market")
    public void market(ProductVO productVO){
        log.info("입력받은 값: " + productVO);
...
//    연산은 VO에서
    ...
    @GetMapping("inputMath")
    public void inputMath(TaskVO taskVO){

    }

    //post방식으로 전달한 경우, return 경로를 상위경로를 반드시 넣어줬다 왤까
    @PostMapping("score")
    public String score(TaskVO taskVO){
        return "ex/score";
    }
...
//    로그인 페이지 이동
    ...
    @PostMapping("signIn")
    public String signIn(@ModelAttribute("id") UserVO userVO){
        log.info("==================================================");
        log.info("이건 userVO객체에 담긴거야(혹시 주소값?) : "+userVO);   //아ㅠ니, UserVO(id=admin, pasword=asdfasdf)로 나옴!!
